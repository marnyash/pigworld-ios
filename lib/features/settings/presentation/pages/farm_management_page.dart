		...
		return Scaffold(
			...
			body: RefreshIndicator(
				...
				child: ListView(padding: const EdgeInsets.all(20), children: [
					Text(session?.selectedFarm?.name ?? 'Your farm', style: Theme.of(context).textTheme.headlineSmall),
					...
					if (isOwner && session?.selectedFarm?.inviteCode != null) ...[
						...
						Card(child: ListTile(
							...
							subtitle: Text('Share this with a farm manager or worker so they can join at sign-up: ${session!.selectedFarm!.inviteCode}'),
							trailing: IconButton(
								tooltip: 'Copy invite code',
								icon: const Icon(Icons.copy_outlined),
								onPressed: () {
									Clipboard.setData(ClipboardData(text: session.selectedFarm!.inviteCode!));
					...
					access.when(
						...
						error: (error, _) => Padding(padding: const EdgeInsets.all(16), child: Text('Could not load farm members: $error')),
								...
								if (isOwner && member.role != UserRole.farmOwner)
									Card(child: Padding(
										...
										child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
											const Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Text('Policies')),
